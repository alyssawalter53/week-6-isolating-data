...
```
select(babynames, ends_with("n")) will not select the n and name columns together.
...
filter(babynames, n ==15| n==16 & year==1980)
...
```{r}
arrange(babynames, n)  #smallest value of n
arrange(babynames, desc(n)) #largest value of n
```
...
babynames%>%
  filter(name =="Alyssa" & sex =="F")%>%
  select(year, prop)%>%
  ggplot(aes(x = year, y = prop))+
  geom_line(size = 1)+
  labs(title = "Proportion of Babies named Alyssa Over Time",
       x = "Time (years)",
